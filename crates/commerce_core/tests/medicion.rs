...
#[test]
#[ignore = "medición, no corrección"]
fn xlsx_contra_columnar_como_formato_intermedio() {
    // La comparación que decide la arquitectura: el mismo DataFrame escrito y
    // releído por los dos caminos. XLSX paga serializar a XML, comprimir,
    // descomprimir y parsear; Arrow IPC copia buffers.
    const FILAS: usize = 500_000;
    const COLUMNAS: usize = 12;
    ...
    let escribir_xlsx = mediana(|| {
        let inicio = std::time::Instant::now();
        let mut e =
            EscritorXlsx::nuevo(tmp.path().join("cmp.xlsx"), OpcionesEscritorXlsx::default()).unwrap();
        e.escribir(&df, Some("Datos")).unwrap();
        e.cerrar().unwrap();
        inicio.elapsed()
    });
    let mut e = EscritorXlsx::nuevo(tmp.path().join("leer.xlsx"), OpcionesEscritorXlsx::default()).unwrap();
    e.escribir(&df, Some("Datos")).unwrap();
    e.cerrar().unwrap();
    let ruta_xlsx = e.ruta.clone();
    let bytes_xlsx = std::fs::metadata(&ruta_xlsx).unwrap().len();
    let leer_xlsx = mediana(|| {
        let inicio = std::time::Instant::now();
        let b = commerce_core::iter_hojas_xlsx(&ruta_xlsx, None, |_: &str| {});
        let t = inicio.elapsed();
        assert!(!b.is_empty());
        t
    });

    let escribir_ipc = mediana(|| {
        let inicio = std::time::Instant::now();
        let mut e = commerce_core::EscritorIpc::nuevo(tmp.path().join("cmp.ipc")).unwrap();
        e.escribir(&df, None).unwrap();
        e.cerrar().unwrap();
        inicio.elapsed()
    });
    let mut e = commerce_core::EscritorIpc::nuevo(tmp.path().join("leer.ipc")).unwrap();
    e.escribir(&df, None).unwrap();
    e.cerrar().unwrap();
    let ruta_ipc = e.ruta.clone();
    let bytes_ipc = std::fs::metadata(&ruta_ipc).unwrap().len();
    let leer_ipc = mediana(|| {
        let inicio = std::time::Instant::now();
        let b = commerce_core::leer_ipc(&ruta_ipc).unwrap();
        let t = inicio.elapsed();
        assert_eq!(b[0].height(), FILAS);
        t
    });

    let ciclo_xlsx = escribir_xlsx + leer_xlsx;
    let ciclo_ipc = escribir_ipc + leer_ipc;
    eprintln!(
        "\n=== FORMATO INTERMEDIO (medianas de 5, {FILAS}x{COLUMNAS}) ===\n\
         XLSX      escribir {escribir_xlsx:.2?} | leer {leer_xlsx:.2?} | ciclo {ciclo_xlsx:.2?} | {:.0} MB\n\
         Arrow IPC escribir {escribir_ipc:.2?} | leer {leer_ipc:.2?} | ciclo {ciclo_ipc:.2?} | {:.0} MB\n\
         GANANCIA  escritura {:.1}x | lectura {:.1}x | CICLO {:.1}x\n",
        bytes_xlsx as f64 / 1048576.0,
        bytes_ipc as f64 / 1048576.0,
        escribir_xlsx.as_secs_f64() / escribir_ipc.as_secs_f64(),
        leer_xlsx.as_secs_f64() / leer_ipc.as_secs_f64(),
        ciclo_xlsx.as_secs_f64() / ciclo_ipc.as_secs_f64(),
    );
}
