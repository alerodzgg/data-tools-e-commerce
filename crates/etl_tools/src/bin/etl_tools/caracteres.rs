...
pub(crate) fn dividir_por_caracteres(ruta_salida: &Path) -> AppResult<()> {
    ...
    /// Lo que el menú ofrece, sin los datos que cada modo necesita después.
    #[derive(Clone, Copy)]
    enum OpcionConteo {
        ColumnaYOrdenar,
        DosHojas,
    ...
    /// La acción ya resuelta. El límite vive DENTRO de las variantes que lo
    /// usan, así que "modo que filtra, pero sin límite" no es un estado
    /// representable y no hay que descartarlo en tiempo de ejecución.
    enum AccionConteo {
        ColumnaYOrdenar,
        DosHojas { limite: u32 },
        ReporteAparte { limite: u32 },
    }
    ...
    let accion = match opcion {
        OpcionConteo::ColumnaYOrdenar => AccionConteo::ColumnaYOrdenar,
        OpcionConteo::DosHojas => AccionConteo::DosHojas {
            limite: preguntar_limite_caracteres()?,
        },
        OpcionConteo::ReporteAparte => AccionConteo::ReporteAparte {
            limite: preguntar_limite_caracteres()?,
        },
    };
    ...
    match accion {
        AccionConteo::ColumnaYOrdenar => modo_columna_y_ordenar(entrada),
        AccionConteo::DosHojas { limite } => modo_dos_hojas(entrada, limite),
        AccionConteo::ReporteAparte { limite } => modo_reporte(entrada, limite),
    }
}

/// El archivo ya leído y las decisiones previas al modo. Viaja entero para que
/// sumar un dato compartido no obligue a tocar las tres firmas.
struct Entrada<'a> {
    archivo: &'a Path,
...
/// Añade la columna de conteo y ordena TODO el archivo por ella.
fn modo_columna_y_ordenar(entrada: Entrada<'_>) -> AppResult<()> {
    let Entrada {
        archivo,
        bloques,
        columnas,
    ...
    if columnas.contains(&COL_CARACTERES.to_string()) {
        app_shell::error(&format!(
...
// Los dos modos que siguen son un FILTRO: cada fila se clasifica solo por su
// propia longitud (`dividir_dentro_fuera`), así que no hace falta juntar el
// archivo entero en un único `DataFrame` (a diferencia del de arriba) — se
// procesa bloque por bloque, sin la copia extra de `armar_desde_bloques`.

/// Divide el archivo en dos hojas: hasta `limite` caracteres y más de `limite`.
fn modo_dos_hojas(entrada: Entrada<'_>, limite: u32) -> AppResult<()> {
    let Entrada {
        archivo,
        bloques,
        columnas,
...
/// Escribe un archivo aparte solo con las filas que superan `limite`.
fn modo_reporte(entrada: Entrada<'_>, limite: u32) -> AppResult<()> {
    let Entrada {
        archivo,
        bloques,
        columnas,
    ...
    // reporte vacío si ninguna fila supera el límite.
    ...
    let ruta = commerce_core::ruta_unica(ruta_salida.join(format!("reporte_mas_de_{limite}_{stem}.xlsx")));
