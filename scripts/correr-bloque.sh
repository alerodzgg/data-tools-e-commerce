...
DATOS="$HOME/datos"
# Una carpeta POR BLOQUE, no una compartida: `aws s3 cp --no-progress --recursive` sube todo
...
if [ ! -f "$DATOS/$BLOQUE.xlsx" ]; then
    aws s3 cp --no-progress "$BUCKET/bloques/$BLOQUE.xlsx" "$DATOS/$BLOQUE.xlsx"
fi
...
#    empezar de cero.
aws s3 cp --no-progress "$BUCKET/resultados/$BLOQUE/" "$SALIDA/" --recursive \
...
aws s3 cp --no-progress "$SALIDA/" "$BUCKET/resultados/$BLOQUE/" --recursive
